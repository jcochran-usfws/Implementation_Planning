...
align = 'llclc') %>%
  ...
  footnote(symbol  = "Priority designation was assigned based on rankings from the risk-based prioritization framework, supporting ongoing invasion fronts, and other AIS program objectives.", threeparttable = T) 
...
## Read in shapefiles
 ...
 cropped <- st_crop(Lake_sf, xmin = -74, xmax = -81,
                                    ymin = 41, ymax = 45.4)
 ...
 #US_cropped <- st_crop(US_sf, xmin = -73, xmax = -81, ymin = 42, ymax = 45.4)
...
\begin{enumerate}
\item Surveillance efforts along the St. Lawrence River near Massena, NY will be ongoing as Tench (\textit{Tinca tinca}) continue to undergo a range expansion into the upper St. Lawrence River. This work is a product of a binational working group focused on tench detentions initiated through the Great Lakes Fish Commission. Partners include: New York State, St. Regis Mowhawk Tribe, DFO, McGill University, The River Institute, and Quebec Ministry of Natural Resources and Forests. This effort is targeted towards littoral habitats that have been suggested to be preferred by Tench and therefore will not be subscribed to the 3-year sampling regime. Massena, NY maintains a High priority status due to the range expansion of Tench and binational partner interest in monitoring that invasion front.

\item Although there will not be any dedicated crayfish or benthic invertebrate survey effort, further support of the Buffalo Park Schools Red Swamp Crayfish (\textit{Procambarus clarkii}) infestation will result in a few days of effort around the perimeter of the known established population to determine any potential spread.  
\end{enumerate}
