...
\begin{itemize}
\item\textbf{Broad-Spectrum:} Monitoring will focus on AIS watchlist species across high priority locations (HPLs) comprised of harbors, rivers, and embayments of the lower Great Lakes as determined by a \href{https://cfpub.epa.gov/si/si_public_record_Report.cfm?dirEntryId=350136&Lab=CCTE}{risk-based prioritization framework for Aquatic Invasive Species (AIS) in the Great Lakes basin}. Highest priority HPLs will get the largest sample efforts, with smaller sample allocations trickling down across the other lower ranked priority sites. 
\item\textbf{Species-Specific:} Monitoring will also focus on particular AIS species while still employing a multi-taxa approach.
\begin{itemize}
...
In 2025, the Lower Great Lakes Fish and Wildlife Conservation Office (LGLFWCO) is looking for partners to assist in metabarcoding sample collection. Locations where assistance may be needed, and the number of samples to be collected by location (i.e., combined USFWS and partner effort) is summarized in Figure 1 and Table 1. Partners should review the table and determine if they have capacity to assist and if any of the locations selected are of interest to their office for AIS or other species purposes. Data from samples collected by partners will be shared when data is available and will contain both native and non-native taxa detected at a given sample site.

If partners have interest in other water bodies in the Great Lakes basin for AIS those areas could potentially be sampled if they align with Great Lakes EDM programmatic goals. If there are additional partner requests the LGLFWCO will review those requests, determine if the areas requested align with programmatic objectives, and readjust sample allocations as needed. Extra areas requested that are not selected for sampling this year can be re-evaluated for future years sampling.

For partners willing to assist with sampling efforts in 2025, the LGLFWCO will provide materials for collection, including: pre-QR code labeled Smith-Root filter packs, a Smith-Root eDNA sampling unit, a step-by-step field SOP, and a app for sample data collection submissions. Training opportunities can also be provided if requested. A Field Map and digital data collection form (Survey123) have already been set up for this program in which partners can access and use during sample collections. A copy of the SOP for the current data workflow can be found here: \href{https://fws.maps.arcgis.com/home/item.html?id=050217d0979e4fbdac761d660023a4e2}{eDNA Projects Survey123 Form User Instructional Manual.} A few example survey designs are included at the end of this document to provided a general idea of what sample locations may look like from a individual location perspective - sample points are subject to change. 

Participating partners will also participate in a reciprocal Data Sharing Notice prior to sample collections that outlines what data needs collected and the data disposition. The USFWS is bound to comply with the OPEN Government Data Act and therefore will make all results publicly available following a communications protocol window that gives the governing waterbodies jurisdictional entity time to review DNA detections before data is released. These details, along with others, are apart of the Data Sharing Notice process. 

\end{flushleft}

\newpage

\large\textbf{AIS Metabarcoding Surviellance Locations and Anticipated Sampling Effort}
\vspace{1.5cm}
...
#| fig-height: 7.5
#| fig-width: 9.5
#| fig-cap: "Map of AIS monitoring locations across the lower Great Lakes with their corresponding priority level (Red: High, Orange: Moderate, Blue: Low)."
...
EDM_tbl <- partnerB %>% 
  rename("Priority*" = Priority, "Sample Allocation" = Metabarcoding_all) %>% 
  arrange(Basin, `Priority*`) %>% 
  select(-Basin, -x, -y, -Purpose, -`Fish Rank`) %>% 
  #select(2, 1, 3, 4, 5, 6) %>% 
  adorn_totals(,,) %>% 
  kable(., format = "latex", digits = 2, booktabs = TRUE, longtable = FALSE, caption = "AIS metabarcoding monitoring locations and associated sample effort across the lower Great Lakes.", align = 'lcc') %>%
  ...
  footnote(symbol  = "Priority designation was assigned based on rankings from both a risk-based prioritization framework and species-specific targeting needs.", threeparttable = T) 
